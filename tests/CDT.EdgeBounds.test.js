...
import { writeEdgeBounds, separatedEdges, separatedPoint } from '../src/core/edgeBounds.js';

it( 'does not reject points or segments accepted by the original distance calculation at floating-point boundaries', () => {
	...
	for ( let i = 0; i < 100000; i ++ ) {
		...
		if ( i % 3 === 0 ) {
			...
			b.start.copy( a.start ); b.end.copy( a.end ); b.start.y = adjacent( b.start.y, 1 ); b.end.y = adjacent( b.end.y, 1 );

		}

		if ( i % 5 === 0 ) {

			p.copy( a.end ); p.x = adjacent( p.x, - 1 );
		...
		if ( i % 17 === 0 ) {
			...
			a.start.set( 1e16, 0, 0 ); a.end.set( 1, 0, 0 ); b.start.set( 0, 0, 0 ); b.end.set( 0, 1, 0 ); p.set( 0, 0, 0 );
	...
	expect( result.falseRejects ).toEqual( [] );
	expect( result.segmentComparisons + result.pointComparisons ).toBe( 2000000 );
	expect( result.naiveCancellationCounterexample.closest1.x ).toBe( 0 );
	expect( result.naiveCancellationCounterexample.bounds[ 0 ] ).toBe( 0 );
