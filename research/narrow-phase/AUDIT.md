> 当前交接：镜像、affine、向量池、稳健 incircle 和距离下界已按独立实验 commit 放入根目录源码；历史完整快照已归档。当前提交顺序和验收边界见 [README](README.md)。下文记录原研究过程，不能把旧版本状态或旧计时当作当前提交结果。

> 交接状态：PR #2 已合入 main；本实验分支基于 `8a53705c`。下文按原实验发生顺序保留历史基线和结论修正。正式实施顺序与复现命令见 [README](README.md)。原始文件可通过 manifest 和 archives 校验。
...
[CDTTriangleSplitter.js:20](archives/historical-sources.tar.gz) 只清理 `_paramPool`，未清理模块级 `_vectorPool`；后者每次 triangulation 继续保留所有投影向量。Evaluator reset、输出 geometry.dispose 和 GC 都不能解除这些强引用。
...
[复现脚本](scripts/audit/csg-agent/pool-leak.mjs)。

### P1：singleton 逻辑跨过真实切割边界

[PR head operations.js:370](archives/historical-sources.tar.gz) 把所有“非共面、仍只有一个三角形”的情况归回原 half-edge component。当切割沿已有边发生时，也不会产生新三角形，但该边仍必须分隔内外区域。
...
[复现脚本](scripts/review/minimal.mjs)。

### P1：镜像变换导致共面分类错误

[PR head operations.js:412](archives/historical-sources.tar.gz) 比较经过变换后的三角形法线，但未修正负 determinant 导致的 winding 翻转。
...
[复现脚本](scripts/audit/csg-agent/correctness-probes.mjs)、[隔离控制](scripts/audit/csg-agent/mirror-fix-probe.mjs)。

### P1：结果矩阵从 affine 降为 TRS，丢失 shear

输入 [narrow-phase.js:589](archives/historical-sources.tar.gz) 明确保留原始 affine matrix；CSG [Evaluator.js:144](archives/historical-sources.tar.gz) 将结果矩阵分解为 position/quaternion/scale。消费者随后在 [1072](archives/historical-sources.tar.gz) 调用 updateMatrixWorld，覆盖精确结果矩阵。
...
[fork 复现](scripts/audit/root/shear-fork-repro.mjs)。

### P2：库生命周期与复用接口存在缺陷

- [Brush.disposeCacheData:118](archives/historical-sources.tar.gz) 清缓存却保留 `_hash`。下一次 prepareGeometry 提前返回，随后 bvhcast 对 null 抛异常。已复现；当前消费者不通过此方法复用同一 Brush，优先级低于前述路径。
- [GeometryBuilder:116](archives/historical-sources.tar.gz) 遍历不存在的 `attributeData.attributes`，且 `.delete` 不是普通对象方法。Evaluator 首次运行后改变 attributes 不会正确清除旧字段。本次 position-only 实验均用全新 Evaluator。
- [GeometryBuilder:312](archives/historical-sources.tar.gz) 用 TypedArray `.type` 比较构造类型，实际为 undefined，因此重复提供 target 仍重新分配。改为真正容量复用之前，消费者必须按 live indices/drawRange 读取顶点；目前 `getObjectWorldVertices` 读取全部 position.count，直接引入更大容量的复用缓冲会污染尺寸。
...
- 复用已计算的共面交线：[operationsUtils:79](archives/historical-sources.tar.gz) 与 93 对同一 triangle pair 重复计算，中间没有修改结果数组。消除重复计算通过样本 parity，速度差在小幅波动范围。
...
- [汇总结果](archives/evidence-root.tar.gz)
- [输入 SHA-256](archives/evidence-root.tar.gz)
- [CPU profile](archives/evidence-root.tar.gz)
- [候选采样脚本](scripts/audit/root/collect-broad.mjs)
- [完整报告及原始几何验证脚本](scripts/audit/root/verify-run.mjs)
- [10 万组谓词差分](scripts/audit/root/predicate-parity.mjs)
- [500 对原版结果](archives/evidence-root.tar.gz)
- [500 对候选结果](archives/evidence-root.tar.gz)
...
- [结果、全部轮次和 parity 检查](archives/evidence-full-project-bench.tar.gz)
- [输入目录与机器配置](archives/evidence-full-project-bench.tar.gz)
- [源码与依赖 SHA 清单](archives/evidence-full-project-bench.tar.gz)
- [57 个输入文件及仓库状态复核](archives/evidence-full-project-bench.tar.gz)
- [完整流水线执行脚本](scripts/audit/full-project-bench/run-full.mjs)
- [顺序执行与 RSS 采样脚本](scripts/audit/full-project-bench/run-queue.py)
- [完整报告比较脚本](scripts/audit/full-project-bench/compare.py)
- [BVG 慢批次与精确候选](archives/evidence-full-project-bench.tar.gz)
- [BVG 逐对 Trace 和三角形数量](archives/evidence-full-project-bench.tar.gz)
- [BVG 慢批次 CPU profile](archives/evidence-full-project-bench.tar.gz)
...
- [完整项目对照及每轮校验](archives/evidence-general-optimizations.tar.gz)
- [原型冻结清单与新增验证项目](archives/evidence-general-optimizations.tar.gz)
- [通用距离下界辅助函数](archives/historical-sources.tar.gz)
- [隔离 splitter 调用位置](archives/historical-sources.tar.gz)
- [独立射线和距离性质检查](archives/evidence-general-optimizations.tar.gz)
- [额外浮点边界检查](archives/evidence-general-optimizations.tar.gz)
- [2,640 项 CSG 原始输出对照](archives/evidence-general-optimizations.tar.gz)
- [600 对真实数据原始几何和报告对照](archives/evidence-general-optimizations.tar.gz)
- [水平容差反例](archives/evidence-general-optimizations.tar.gz)
- [垂直上界机会统计，未启用早退](archives/evidence-general-optimizations.tar.gz)
- [库测试日志](archives/evidence-general-optimizations.tar.gz)
- [输入、源码和产品仓库状态复核](archives/evidence-general-optimizations.tar.gz)
...
> 后续修正：上述两项测试使用不符合契约的开放 B，不能证明需要保留 shortcut。此处保留历史解释，正式交接以本文末尾的 singleton 复核为准。
...
> 当前实施边界：singleton 已在分支基线修复，前句对此的要求已撤回；镜像、affine、输入质量和早退诊断语义仍是后续工作。

### 本轮复核产物

- [完整计时、逐轮 CSV/coverage 对照](archives/evidence-vertical-bound.tar.gz)
- [全部执行 CSG 的 shadow 统计](archives/evidence-vertical-bound.tar.gz)
- [认证算法与边界论证](scripts/audit/vertical-bound/numerics/PROOF.md)
- [独立精确算术检查](archives/evidence-vertical-bound.tar.gz)
- [Duplicate 与多 brush 契约检查](archives/evidence-vertical-bound.tar.gz)
- [普通 shear 的独立阈值反例](archives/evidence-vertical-bound.tar.gz)
- [真实 WBDG 顶点和索引复核](scripts/audit/vertical-bound/numerics/REAL-FORENSICS.md)
- [通用 whole-pair 判定原型](scripts/audit/vertical-bound/policy.mjs)
- [控制组库测试，含两项失败](archives/evidence-vertical-bound.tar.gz)
- [输入、运行源码与产品状态复核](archives/evidence-vertical-bound.tar.gz)

## PR #2 最新 singleton 修复复核（2026-09-08）

> 以下记录修复复核当时的远端状态。交接时 PR #2 已合入 main，分支基线为 `8a53705c`。
...
证据：[汇总与源码/产品状态](archives/evidence-review.tar.gz)、[库测试](archives/evidence-review.tar.gz)、[lint](archives/evidence-review.tar.gz)、[解析 sweep](archives/evidence-review.tar.gz)、[镜像复现](archives/evidence-review.tar.gz)、[shear 复现](archives/evidence-review.tar.gz)。
