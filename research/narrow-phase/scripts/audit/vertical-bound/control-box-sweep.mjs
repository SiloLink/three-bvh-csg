...
import * as head from '__CSG_RESEARCH_ROOT__/audits/csg-narrow-20260907/root/variants/vertical-control/csg/src/index.js';
...
for (const cdt of [false,true]) for (const segments of [1,2]) for(const x of xs) for(const y of [0,0.5,1]) for(const bx of [[1,1,1],[2,2,2],[0.5,0.5,0.5]]) {
 ...
 count++;
 for(let i=0;i<ops.length;i++) { const hf=Math.abs(h[i]-expected[i])>1e-5,bf=Math.abs(b[i]-expected[i])>1e-5; controlFailures+=Math.abs(c[i]-expected[i])>1e-5; headFailures+=hf; baseFailures+=bf; if(hf&&!bf) regressions.push({cdt,segments,pos,bx,op:ops[i],expected:expected[i],head:h[i],base:b[i]}); }
}
console.log(JSON.stringify({count,headFailures,baseFailures,controlFailures,regressionCount:regressions.length,byMode:{legacy:regressions.filter(r=>!r.cdt).length,cdt:regressions.filter(r=>r.cdt).length}},null,2));
