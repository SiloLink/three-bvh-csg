...
const examples = [];
const errors = [];
const shears = [1, 10, 100, 1000, 1e4, 1e5, 1e6, 1e7, 1e8, 1e9, 1e10, 1e11, 1e12];
for (const shear of shears) {
  for (const margin of [1e-14, 1e-12, 1e-10, 1e-8, 1e-6]) {
    ...
    const b = brush(box(1, 1, height), new T.Matrix4());
    ...
    try {
      ...
      if (range.heightMm > 10.1 && volumeMm3 > 1) {
        examples.push({ shear, margin, inputDimensionsA: [4, 4, 4 * shear + 1], inputDimensionsB: [1, 1, height],
...
fs.writeFileSync(new URL('./output-overrun-search.json', import.meta.url), JSON.stringify(report, null, 2) + '\n');
